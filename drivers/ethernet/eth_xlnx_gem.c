...
static void eth_xlnx_gem_isr(const struct device *dev)
{
	...
	if (reg_val & ETH_XLNX_GEM_IXR_ERRORS_MASK) {
		LOG_ERR("%s error bit(s) set in Interrupt Status Reg.: %d", dev->name, reg_val);
	}
...
static int eth_xlnx_gem_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	LOG_DBG("GEM sending the packet");
	...
	if (!dev_data->started || dev_data->eff_link_speed == LINK_DOWN ||
		...
		// check the error
		if (!dev_data->started) {
			LOG_ERR("Data not started");
		}
		if (dev_data->eff_link_speed == LINK_DOWN) {
			LOG_ERR("Link down");
		}
		if (!net_if_flag_is_set(dev_data->iface, NET_IF_UP)) {
			LOG_ERR("Iface down!");
		}
...
static void eth_xlnx_gem_configure_clocks(const struct device *dev)
{
	...
	// print the SLCR MIO conf. registers
	// LOG_DBG("Tx regs: %u, %u, %u, %u, %u,
	// %u",sys_read32(0xF8000740),sys_read32(0xF8000744),sys_read32(0xF8000748),sys_read32(0xF800074C),sys_read32(0xF8000750),sys_read32(0xF8000754));
	// LOG_DBG("Rx regs: %u, %u, %u, %u, %u,
...
#elif defined(CONFIG_SOC_FAMILY_XILINX_ZYNQ7000)
	// try to change the clock reg address
	LOG_DBG("SLCR write prot. status: %u", sys_read32(0xF800000C));
	clk_ctrl_reg = sys_read32(dev_conf->clk_ctrl_reg_address);
	...
	sys_write32((unsigned int)('5'), 0xE0001030);
	sys_write32(clk_ctrl_reg, dev_conf->clk_ctrl_reg_address);
	// sys_write32(clk_ctrl_reg, 0x7ff140);
...
static void eth_xlnx_gem_set_initial_nwcfg(const struct device *dev)
{
	...
	LOG_DBG("Net config register: %u", reg_val);
...
static void eth_xlnx_gem_init_phy(const struct device *dev)
{
	...
	LOG_INF("%s attempting to initialize associated PHY", dev->name);
	...
	if (detect_rc == 0 && dev_data->phy_id != 0x00000000 && dev_data->phy_id != 0xFFFFFFFF &&
		...
		LOG_WRN("%s compatible PHY detected", dev->name);
	} else {
...
static void eth_xlnx_gem_configure_buffers(const struct device *dev)
{
	...
	LOG_DBG("Set up the DMA buffers, size is %u",sizeof(struct eth_xlnx_gem_bd));
}
